...
namespace FastTool.HotKey
{
    ...
    public class KeyboardHookManager
    {
        ...
        /// </summary>
        public readonly HashSet<ModifierKeys> _downModifierKeys;
        /// <summary>
        ...
        private KeybindStruct prewkeybind;
        #endregion
        ...
        public Guid RegisterHotkey(ModifierKeys[] modifiers, int virtualKeyCode, Action action)
        {
            ...
            var keybind = new KeybindStruct(null, modifiers, virtualKeyCode, keybindIdentity);
        ...
        public Guid RegisterHotkey(ModifierKeys prewmodifiers, int prewvirtualKeyCode, ModifierKeys[] modifiers, int virtualKeyCodes, Action action)
        {
            var allModifiers = Enum.GetValues(typeof(ModifierKeys)).Cast<ModifierKeys>().ToArray();

            // Get the modifiers that were chained with bitwise OR operation as an array of modifiers
            var prewselectedModifiers = allModifiers.Where(modifier => prewmodifiers.HasFlag(modifier)).ToArray();
        ...
        public Guid RegisterHotkey(ModifierKeys prewmodifiers, int prewvirtualKeyCode, ModifierKeys modifiers, int virtualKeyCode, Action action)
        {
            ...
            return RegisterHotkey(prewselectedModifiers, prewvirtualKeyCode, selectedModifiers, virtualKeyCode, action);
        }

        public Guid RegisterHotkey(ModifierKeys[] prewmodifiers, int prewvirtualKeyCodes, ModifierKeys modifiers, int virtualKeyCode, Action action)
        {
            var allModifiers = Enum.GetValues(typeof(ModifierKeys)).Cast<ModifierKeys>().ToArray();
        ...
        {
            var keybindIdentity = Guid.NewGuid();
            KeybindStruct prewkeybind = new KeybindStruct(null, prewmodifiers, prewvirtualKeyCode);
            var keybind = new KeybindStruct(prewkeybind, modifiers, virtualKeyCode, keybindIdentity);
            if (this._registeredCallbacks.ContainsKey(keybind))
            {
                throw new HotkeyAlreadyRegisteredException();
            }
        ...
        {
            var keybind = new KeybindStruct(prewkeybind, modifiers, virtualKeyCode);
        ...
        /// <exception cref="HotkeyNotRegisteredException">Thrown when the given key combination is not registered</exception>
        public void UnregisterHotkey(ModifierKeys[] prewmodifiers, int prewvirtualKeyCode, ModifierKeys[] modifiers, int virtualKeyCode)
        {
            KeybindStruct prewkeybind = new KeybindStruct(null, prewmodifiers, prewvirtualKeyCode);
            var keybind = new KeybindStruct(prewkeybind, modifiers, virtualKeyCode);
        ...
        private void HandleKeyPress()
        {
            ...
            if (_downModifierKeys.Count > 0 && _downKeys.Count > 0)
            {
                prewkeybind = new KeybindStruct(null, this._downModifierKeys, this._downKeys.Count > 0 ? this._downKeys.Last() : 0);
            }
            ...
            currentKey = new KeybindStruct(null, this._downModifierKeys, this._downKeys.Count > 0 ? this._downKeys.Last() : 0);
            ...
            if (this._registeredCallbacks.TryGetValue(currentKey, out var _callback))
            {
                _callback.Invoke();
                return;
            }
        ...
        private void HandleSingleKeyboardInput(object keyboardParamsObj)
        {
            ...
            if (wParam == (IntPtr)WM_KEYDOWN || wParam == (IntPtr)WM_SYSKEYDOWN)
            {
                ...
                if (modifierKey != null && !this._downModifierKeys.Contains(modifierKey.Value))
                {
                ...
                if (!this._downKeys.Contains(vkCode) && modifierKey == null)
                {
